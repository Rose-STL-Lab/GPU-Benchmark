...
class Dataset(data.Dataset):
    def __init__(self, samples, indices, input_length, mid, output_length, stack_x):
    ...
    def __getitem__(self, index):
        ...
        y = self.samples[ID][self.mid:(self.mid+self.output_length)]
        if self.stack_x:
            x = self.samples[ID][(self.mid-self.input_length):self.mid].reshape(-1, y.shape[-2], y.shape[-1])
        else:
            x = self.samples[ID][(self.mid-self.input_length):self.mid]
